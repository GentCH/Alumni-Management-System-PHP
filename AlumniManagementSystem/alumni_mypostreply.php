...
<br /><br />
<hr color="peru" size="2" />
<?php
if(isset($_POST['posts']))
{
	echo "<p class=pp1>My Posts:</p>";
	echo "<table class=ptb1 cellspacing=15 align=center>";
	$sql="SELECT pi_full_name FROM alumniinfo WHERE al_id='$userid'";
	$result=$conn->query($sql);
	if ($result->num_rows > 0) 
	{
		while($row=$result->fetch_assoc())
		{
			$fullname=$row['pi_full_name'];
...
if(isset($_POST['replies']))
{
	...
	echo "<table class=ptb1 cellspacing=15 align=center>";
	$sql3="SELECT pi_full_name FROM alumniinfo WHERE al_id='$userid'";
